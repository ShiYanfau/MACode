...
def test(model, device, test_loader, criterion, epoch=None, checkpoint=None, set="Test"):
    ...
    """compare loss and update the best"""
    if set == "Validation":
        # current model better than the historical best
...
def run(args):
    ...
    # TODO: adjust folder
    dataset = datasets.CIFAR10('./dataset/CIFAR10_train-val', download=True, train=True, transform=transform)
    ...
    # TODO: adjust folder
    testset = datasets.CIFAR10('./dataset/CIFAR10_test', download=True, train=False, transform=transform)
    ...
    best_checkpoint = {"best_loss": float('INF'),
                       "best_epoch": 0,
                       "best_model": model}
    ...
    test(best_checkpoint['best_model'], device, testloader, criterion)

    """save the best model"""
    if args.save_model:
        torch.save(best_checkpoint, "./best_{model_name}_e{epoch}_score{val_loss}.pt".format(model_name=args.model,
